...
void nfs_tcp_handshake_init(struct nfs_tcp_handshake *ctx, uint16_t local_port,
...
int nfs_tcp_parse(const uint8_t *buf, size_t len, struct nfs_tcp_hdr *out) {
...
static size_t serialize_hdr(const struct nfs_tcp_hdr *h, uint8_t *buf, size_t len) {
...
size_t nfs_tcp_build_syn(struct nfs_tcp_handshake *ctx, uint8_t *buf, size_t len) {
...
size_t nfs_tcp_build_synack(struct nfs_tcp_handshake *ctx, const struct nfs_tcp_hdr *syn_hdr,
...
size_t nfs_tcp_build_ack(struct nfs_tcp_handshake *ctx, const struct nfs_tcp_hdr *synack_hdr,
                         uint8_t *buf, size_t len) {
    if (!ctx || !synack_hdr || !buf || len < sizeof(struct nfs_tcp_hdr))
        return 0;

    /* Must be a SYN-ACK. */
    if ((synack_hdr->flags & (NFS_TCP_SYN | NFS_TCP_ACK)) != (NFS_TCP_SYN | NFS_TCP_ACK))
        return 0;
...
uint16_t nfs_tcp_checksum_pseudo(const uint8_t *src_ip, const uint8_t *dst_ip,
                                 const uint8_t *tcp_buf, size_t tcp_len) {
