...
{
    if (phase >= NFS_BBR_PROBE_BW_PHASES) return 1000;
...
static void bw_filter_add(struct nfs_bbr_bw_filter *f,
                           uint64_t rate, uint64_t timestamp)
{
    size_t idx = f->head;
    f->samples[idx] = rate;
...
{
    if (bbr->state != NFS_BBR_STARTUP) return;
...
{
    if (bbr->state != NFS_BBR_DRAIN) return;
...
{
    if (bbr->state == NFS_BBR_PROBE_RTT) return;
...
uint32_t nfs_bbr_pacing_gain(const struct nfs_bbr *bbr)
{
    if (!bbr) return 1000;
    switch (bbr->state) {
    case NFS_BBR_STARTUP:   return NFS_BBR_GAIN_STARTUP;
    case NFS_BBR_DRAIN:     return NFS_BBR_GAIN_DRAIN;
    case NFS_BBR_PROBE_BW:  return nfs_bbr_probe_bw_gain(bbr->probe_bw_phase);
    case NFS_BBR_PROBE_RTT: return NFS_BBR_GAIN_PROBE_RTT;
    default:                return 1000;
    }
}
...
uint32_t nfs_bbr_cwnd_gain(const struct nfs_bbr *bbr)
{
    if (!bbr) return 1000;
    switch (bbr->state) {
    case NFS_BBR_STARTUP: return 2885;  /* same as pacing in startup */
    case NFS_BBR_DRAIN:   return 2885;  /* keep cwnd high to avoid drops */
    case NFS_BBR_PROBE_BW: return 2000; /* 2x BDP */
    case NFS_BBR_PROBE_RTT: return 1000;
    default: return 1000;
    }
}
...
uint32_t nfs_bbr_compute_cwnd(const struct nfs_bbr *bbr)
{
    ...
    if (target < min_cwnd) target = min_cwnd;
...
{
    switch (state) {
    case NFS_BBR_STARTUP:   return "STARTUP";
    case NFS_BBR_DRAIN:     return "DRAIN";
    case NFS_BBR_PROBE_BW:  return "PROBE_BW";
    case NFS_BBR_PROBE_RTT: return "PROBE_RTT";
