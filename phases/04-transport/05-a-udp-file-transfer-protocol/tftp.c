...
int nfs_tftp_build_rrq(uint8_t *buf, size_t buf_max, uint16_t opcode, const char *filename,
                       const char *mode) {
    if (!buf || !filename || !mode)
        return -1;
    if (opcode != NFS_TFTP_OP_RRQ && opcode != NFS_TFTP_OP_WRQ)
        return -1;

    size_t fn_len = strlen(filename);
    size_t mode_len = strlen(mode);
    if (fn_len == 0 || fn_len >= NFS_TFTP_FILENAME_MAX)
        return -1;
    if (mode_len == 0 || mode_len >= NFS_TFTP_MODE_MAX)
        return -1;
...
int nfs_tftp_build_data(uint8_t *buf, size_t buf_max, uint16_t block, const uint8_t *data,
                        uint16_t data_len) {
    if (!buf)
        return -1;
    if (data_len > NFS_TFTP_DATA_MAX)
        return -1;
    if (data_len > 0 && !data)
        return -1;
...
int nfs_tftp_build_error(uint8_t *buf, size_t buf_max, uint16_t code, const char *msg) {
    if (!buf || !msg)
        return -1;

    size_t msg_len = strlen(msg);
    if (msg_len >= NFS_TFTP_ERRMSG_MAX)
        return -1;
...
int nfs_tftp_parse(const uint8_t *buf, size_t len, struct nfs_tftp_packet *out) {
    if (!buf || !out || len < 2)
        return -1;
    ...
    case NFS_TFTP_OP_ERROR: {
        if (len < 5)
            return -1; /* at least opcode + errcode + NUL */
        uint16_t code_net;
...
const char *nfs_tftp_error_name(uint16_t code) {
    switch (code) {
    case NFS_TFTP_ERR_UNDEFINED:
        return "Not defined";
    case NFS_TFTP_ERR_NOT_FOUND:
        return "File not found";
    case NFS_TFTP_ERR_ACCESS:
        return "Access violation";
    case NFS_TFTP_ERR_DISK_FULL:
        return "Disk full";
    case NFS_TFTP_ERR_ILLEGAL_OP:
        return "Illegal operation";
    case NFS_TFTP_ERR_UNKNOWN_TID:
        return "Unknown transfer ID";
    case NFS_TFTP_ERR_EXISTS:
        return "File already exists";
    case NFS_TFTP_ERR_NO_USER:
        return "No such user";
    default:
        return "UNKNOWN";
    }
