...
int main(void)
{
    ...
    /* Build and parse RRQ */
    n = nfs_tftp_build_rrq(buf, sizeof(buf), NFS_TFTP_OP_RRQ,
                           "test.txt", "octet");
    ...
    n = nfs_tftp_build_error(buf, sizeof(buf),
                             NFS_TFTP_ERR_NOT_FOUND, "File not found");
    printf("\nERROR (%d bytes): ", n);
    hexdump(buf, (size_t)n);
    ...
    if (nfs_tftp_parse(buf, (size_t)n, &pkt) == 0) {
        printf("  opcode=%s errcode=%u (%s) msg='%s'\n",
               nfs_tftp_opcode_name(pkt.opcode),
               pkt.u.error.code,
               nfs_tftp_error_name(pkt.u.error.code),
               pkt.u.error.msg);
    }
