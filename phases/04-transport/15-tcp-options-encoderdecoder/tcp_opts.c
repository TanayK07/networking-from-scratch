                       ...
                       size_t *nfound)
{
    ...
    while (pos < len && count < max_opts) {
        ...
        case NFS_TCPOPT_SACK: {
            ...
            if (block_bytes % 8 != 0) return -1;
            size_t nblocks = block_bytes / 8;
            if (nblocks > NFS_TCPOPT_MAX_SACK_BLOCKS) return -1;
...
void nfs_tcp_opt_format(const struct nfs_tcp_option *opt, char *buf, size_t sz)
{
    ...
    case NFS_TCPOPT_SACK: {
        ...
        for (size_t i = 0; i < opt->data.sack.nblocks && (size_t)off < sz; i++) {
            if (i > 0) off += snprintf(buf + off, sz - (size_t)off, ", ");
            off += snprintf(buf + off, sz - (size_t)off, "%u-%u",
                            opt->data.sack.blocks[i * 2],
    ...
    case NFS_TCPOPT_TIMESTAMPS:
        snprintf(buf, sz, "TS val=%u ecr=%u",
                 opt->data.timestamps.ts_val,
