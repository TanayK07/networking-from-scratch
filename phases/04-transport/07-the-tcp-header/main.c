...
static uint16_t parse_flags_str(const char *s) {
    uint16_t flags = 0;
    if (strstr(s, "FIN")) flags |= NFS_TCP_FIN;
    if (strstr(s, "SYN")) flags |= NFS_TCP_SYN;
    if (strstr(s, "RST")) flags |= NFS_TCP_RST;
    if (strstr(s, "PSH")) flags |= NFS_TCP_PSH;
    if (strstr(s, "ACK")) flags |= NFS_TCP_ACK;
    if (strstr(s, "URG")) flags |= NFS_TCP_URG;
    if (strstr(s, "ECE")) flags |= NFS_TCP_ECE;
    if (strstr(s, "CWR")) flags |= NFS_TCP_CWR;
    return flags;
}
...
int main(int argc, char **argv) {
    if (argc == 2) {
        ...
        printf("  Data offset:  %u (header = %u bytes)\n",
               nfs_tcp_data_offset(&hdr), nfs_tcp_data_offset(&hdr) * 4);
