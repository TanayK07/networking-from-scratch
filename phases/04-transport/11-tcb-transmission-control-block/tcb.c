...
static void fmt_addr(uint32_t addr, char *buf, size_t sz) {
    snprintf(buf, sz, "%u.%u.%u.%u", (addr >> 24) & 0xFF, (addr >> 16) & 0xFF, (addr >> 8) & 0xFF,
             addr & 0xFF);
}
...
void nfs_tcb_format(const struct nfs_tcb *tcb, char *buf, size_t sz) {
    ...
    snprintf(buf, sz,
             ...
             "rcv_nxt=%u rcv_wnd=%u",
             la, tcb->local_port, ra, tcb->remote_port, tcb->snd_una, tcb->snd_nxt, tcb->snd_wnd,
...
int nfs_tcb_matches(const struct nfs_tcb *tcb, uint32_t src_addr, uint16_t src_port,
    ...
    return (tcb->remote_addr == src_addr && tcb->remote_port == src_port &&
            tcb->local_addr == dst_addr && tcb->local_port == dst_port)
               ? 1
               : 0;
}
