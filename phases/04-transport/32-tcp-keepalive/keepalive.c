...
int nfs_keepalive_check(struct nfs_keepalive *ka, double now)
{
    ...
    if (ka->probes_sent == 0) {
        ...
        double probe_threshold = ka->idle_timeout +
                                 (double)ka->probes_sent * ka->interval;
                          ...
                          char *buf, size_t sz)
{
    ...
    snprintf(buf, sz,
             ...
             ka->enabled ? "enabled" : "disabled",
             ka->idle_timeout, ka->interval,
             ka->probes_sent, ka->max_probes,
             ka->last_data_time);
}
