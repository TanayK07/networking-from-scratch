/* CLI: compute or verify TCP checksum with IPv4 pseudo-header.
 *
 * Compute mode:
 *   $ ./tcp_csum compute 192.168.1.1 10.0.0.1 "3039005000000001..."
 *
 * Verify mode:
 *   $ ./tcp_csum verify 192.168.1.1 10.0.0.1 "3039005000000001..."
 */
...
int main(int argc, char **argv) {
    if (argc != 5) {
        fprintf(stderr, "usage: %s compute|verify <src_ip> <dst_ip> <hex_tcp_segment>\n", argv[0]);
        return 1;
    }
    ...
    if (strcmp(mode, "compute") == 0) {
        uint16_t cs = nfs_tcp_checksum(src_ip, dst_ip, segment, seg_len);
        printf("Checksum: 0x%04X\n", cs);
        return 0;
    }

    if (strcmp(mode, "verify") == 0) {
        int valid = nfs_tcp_verify_checksum(src_ip, dst_ip, segment, seg_len);
        if (valid) {
            printf("Valid\n");
            return 0;
        } else {
