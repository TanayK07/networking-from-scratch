...
static int tuple_match(const struct nfs_ct_tuple *a,
                       const struct nfs_ct_tuple *b)
{
    /* Forward match */
...
static struct nfs_ct_entry *find_entry(struct nfs_conntrack *ct,
...
nfs_ct_state_t nfs_conntrack_process(struct nfs_conntrack *ct,
                                     ...
                                     time_t now)
{
    if (!ct || !pkt) return NFS_CT_INVALID;
    ...
    if (entry) {
        ...
        if (pkt->protocol == 6) {  /* TCP */
            ...
            if (entry->state == NFS_CT_ESTABLISHED &&
                (pkt->tcp_flags & NFS_CT_TCP_SYN) &&
            ...
            if (entry->state == NFS_CT_NEW) {
                ...
                if (pkt->src_ip == entry->tuple.dst_ip &&
                    pkt->dst_ip == entry->tuple.src_ip) {
                    entry->state = NFS_CT_ESTABLISHED;
                    return NFS_CT_ESTABLISHED;
                }
            }
    ...
    if (pkt->protocol == 6) {
        ...
        if (!(pkt->tcp_flags & NFS_CT_TCP_SYN) ||
            (pkt->tcp_flags & NFS_CT_TCP_ACK)) {
            return NFS_CT_INVALID;
        }
    }
    ...
    if (!slot) return NFS_CT_INVALID;  /* table full */
...
const struct nfs_ct_entry *nfs_conntrack_lookup(
    ...
    const struct nfs_ct_tuple *tuple)
{
    if (!ct || !tuple) return NULL;

    for (int i = 0; i < NFS_CT_MAX_ENTRIES; i++) {
        if (ct->entries[i].active &&
            tuple_match(&ct->entries[i].tuple, tuple))
            return &ct->entries[i];
    }
    return NULL;
}
...
int nfs_conntrack_expire(struct nfs_conntrack *ct, time_t now)
{
    ...
    for (int i = 0; i < NFS_CT_MAX_ENTRIES; i++) {
        if (ct->entries[i].active &&
            (now - ct->entries[i].last_seen) >= ct->timeout) {
...
{
    switch (state) {
    case NFS_CT_NEW:         return "NEW";
    case NFS_CT_ESTABLISHED: return "ESTABLISHED";
    case NFS_CT_RELATED:     return "RELATED";
    case NFS_CT_INVALID:     return "INVALID";
