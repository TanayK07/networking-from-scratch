...
int main(void) {
    ...
    printf("TOS=0x%02X -> ECN: %s, ECT=%d, CE=%d\n", tos,
           nfs_ecn_codepoint_name(nfs_ecn_get_ip(tos)), nfs_ecn_is_ect(tos), nfs_ecn_is_ce(tos));

    int ce_tos = nfs_ecn_mark_ce(tos);
    printf("After CE mark: TOS=0x%02X -> ECN: %s\n", (uint8_t)ce_tos,
           nfs_ecn_codepoint_name(nfs_ecn_get_ip((uint8_t)ce_tos)));

    /* TCP ECN flags */
    printf("\n--- TCP ECN Negotiation ---\n");
    uint16_t syn_flags = nfs_ecn_build_syn_flags();
    printf("SYN flags:          0x%03X (ECE=%d, CWR=%d, SYN=%d)\n", syn_flags,
           (syn_flags & NFS_TCP_FLAG_ECE) ? 1 : 0, (syn_flags & NFS_TCP_FLAG_CWR) ? 1 : 0,
    ...
    printf("SYN-ACK (accept):   0x%03X (ECE=%d, CWR=%d)\n", synack_flags,
           (synack_flags & NFS_TCP_FLAG_ECE) ? 1 : 0, (synack_flags & NFS_TCP_FLAG_CWR) ? 1 : 0);
