...
{
    if (!nfs_ecn_is_ect(tos)) return -1;
...
{
    if (!nego) return NFS_ECN_STATE_UNKNOWN;

    if (!is_syn_ack) {
        /* Processing incoming SYN (we are the responder) */
        if ((tcp_flags & NFS_TCP_FLAG_SYN) &&
            (tcp_flags & NFS_TCP_FLAG_ECE) &&
        ...
        if ((tcp_flags & NFS_TCP_FLAG_SYN) &&
            (tcp_flags & NFS_TCP_FLAG_ACK)) {
            if ((tcp_flags & NFS_TCP_FLAG_ECE) &&
                !(tcp_flags & NFS_TCP_FLAG_CWR)) {
...
{
    switch (ecn) {
    case NFS_ECN_NOT_ECT: return "Not-ECT";
    case NFS_ECN_ECT1:    return "ECT(1)";
    case NFS_ECN_ECT0:    return "ECT(0)";
    case NFS_ECN_CE:      return "CE";
...
{
    switch (state) {
    case NFS_ECN_STATE_UNKNOWN:   return "UNKNOWN";
    case NFS_ECN_STATE_REQUESTED: return "REQUESTED";
    case NFS_ECN_STATE_ACCEPTED:  return "ACCEPTED";
    case NFS_ECN_STATE_REJECTED:  return "REJECTED";
    case NFS_ECN_STATE_ACTIVE:    return "ACTIVE";
