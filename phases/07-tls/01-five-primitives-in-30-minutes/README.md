# 01. Five primitives in 30 minutes

## 1. Problem

Before diving into implementation details, you need a clear mental model of how five primitives in 30 minutes works. Without understanding the design rationale and tradeoffs, you will struggle to debug real systems or make informed architectural decisions.
...
Five primitives in 30 minutes is a fundamental concept in tls & secure transport. Understanding it requires grasping both the high-level design philosophy and the low-level details.
...
When studying five primitives in 30 minutes, pay attention to:
...
The theoretical foundation for five primitives in 30 minutes involves understanding the tradeoffs between different design choices. Key metrics include:
...
```python
"""
five_primitives_in_30_minutes.py -- Five primitives in 30 minutes
"""
import struct
import socket

class PacketBuilder:
    """Build and parse protocol packets for Five primitives in 30 minutes."""
...
def demo():
    payload = b"Hello from Five primitives in 30 minutes"
...
1. **★** Parse a hex dump of a real five primitives in 30 minutes packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes five primitives in 30 minutes packets with full field breakdown.
