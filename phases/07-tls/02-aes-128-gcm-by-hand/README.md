# 02. AES-128-GCM by hand

## 1. Problem

In real networking systems, aes-128-gcm by hand is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that aes-128-gcm by hand involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

AES-128-GCM by hand requires careful attention to binary protocol formats and state management. The implementation must handle network byte order (big-endian) correctly and validate all input before processing.
...
```c
/*
 * aes128gcm_by_hand.c -- AES-128-GCM by hand
 * Compile: gcc -Wall -O2 -o aes128gcm_by_hand aes128gcm_by_hand.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from AES-128-GCM by hand";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for AES-128-GCM by hand passed.\n");
    return 0;
}
```
...
1. **★** Parse a hex dump of a real aes-128-gcm by hand packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes aes-128-gcm by hand packets with full field breakdown.
