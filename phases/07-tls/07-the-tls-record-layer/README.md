# 07. The TLS record layer

## 1. Problem

In real networking systems, the tls record layer is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that the tls record layer involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

The TLS record layer is a core concept in TLS & Secure Transport. Understanding it requires grasping both the design philosophy and the implementation details.

TLS record layer:

```
  +---------------+
  | Content Type  |  1 byte (0x17 = application data, 0x16 = handshake)
  +---------------+
  | Version       |  2 bytes (0x0303 for TLS 1.2/1.3)
  +---------------+
  | Length         |  2 bytes (max 16384 + 256)
  +---------------+
  | Fragment       |  encrypted in TLS 1.3:
  |  (plaintext    |    [content | content_type | zeros] encrypted
  |   or encrypted)|    + authentication tag
  +---------------+
```

In TLS 1.3, all records after ServerHello are encrypted.
Outer content type is always 0x17 (application_data) for encrypted records.
Inner content type (after decryption) reveals the actual message type.
...
```c
/*
 * the_tls_record_layer.c -- The TLS record layer
 * Compile: gcc -Wall -O2 -o the_tls_record_layer the_tls_record_layer.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from The TLS record layer";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for The TLS record layer passed.\n");
    return 0;
}
```
...
1. **\u2605** Parse a hex dump of a real the tls record layer packet and identify every field manually.
...
5. **\u2605\u2605** Write a pcap analyzer that reads capture files and decodes the tls record layer packets with full field breakdown.
