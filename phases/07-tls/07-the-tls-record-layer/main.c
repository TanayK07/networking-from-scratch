...
int main(void) {
    ...
    nfs_tls_alert_build(alert_frag, sizeof(alert_frag), NFS_TLS_ALERT_FATAL,
                        NFS_TLS_ALERT_HANDSHAKE_FAILURE);

    n = nfs_tls_record_build(buf, sizeof(buf), NFS_TLS_CT_ALERT, NFS_TLS_VERSION_12, alert_frag, 2);
    ...
    if (nfs_tls_record_parse(buf, (size_t)n, &rec) > 0) {
        ...
        if (nfs_tls_alert_parse(rec.fragment, rec.length, &alert) == 0) {
            ...
            printf("  description: %s (%u)\n", nfs_tls_alert_desc_name(alert.description),
                   alert.description);
        }
    }
