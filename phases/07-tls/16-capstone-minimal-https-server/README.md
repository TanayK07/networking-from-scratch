# 16. Capstone: minimal HTTPS server

## 1. Problem

This capstone brings together everything from the TLS & Secure Transport phase into a single working project. You will build a complete, functional implementation of minimal HTTPS server that demonstrates mastery of all concepts covered so far.
...
The minimal HTTPS server project integrates multiple components built in previous lessons. The architecture follows a layered design where each module handles one responsibility:
...
```c
/*
 * capstone_minimal_https_server.c -- Capstone: Capstone: minimal HTTPS server
 *
 * Architecture:
 ...
 *
 * Compile: gcc -Wall -O2 -o capstone_minimal_https_server capstone_minimal_https_server.c -lpthread
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{
    printf("Capstone: minimal HTTPS server\n");
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for Capstone: minimal HTTPS server passed.\n");
    return 0;
}
```
