# 08. Linkerd's design choices

## 1. Problem

Before diving into implementation details, you need a clear mental model of how linkerd's design choices works. Without understanding the design rationale and tradeoffs, you will struggle to debug real systems or make informed architectural decisions.
...
Linkerd's design choices is a fundamental concept in service mesh & cloud-native. Understanding it requires grasping both the high-level design philosophy and the low-level details.
...
When studying linkerd's design choices, pay attention to:
...
The theoretical foundation for linkerd's design choices involves understanding the tradeoffs between different design choices. Key metrics include:
...
```python
"""
linkerds_design_choices.py -- Linkerd's design choices
"""
import struct
import socket

class PacketBuilder:
    """Build and parse protocol packets for Linkerd's design choices."""
...
def demo():
    payload = b"Hello from Linkerd's design choices"
...
1. **\u2605** Parse a hex dump of a real linkerd's design choices packet and identify every field manually.
...
5. **\u2605\u2605** Write a pcap analyzer that reads capture files and decodes linkerd's design choices packets with full field breakdown.
