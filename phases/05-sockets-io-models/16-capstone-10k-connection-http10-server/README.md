# 16. Capstone: 10k-connection HTTP/1.0 server

## 1. Problem

This capstone brings together everything from the Sockets & I/O Models phase into a single working project. You will build a complete, functional implementation of 10k-connection HTTP/1.0 server that demonstrates mastery of all concepts covered so far.
...
The 10k-connection HTTP/1.0 server project integrates multiple components built in previous lessons. The architecture follows a layered design where each module handles one responsibility:
...
```c
/*
 * capstone_10kconnection_http10_server.c -- Capstone: Capstone: 10k-connection HTTP/1.0 server
 *
 * Architecture:
 ...
 *
 * Compile: gcc -Wall -O2 -o capstone_10kconnection_http10_server capstone_10kconnection_http10_server.c -lpthread
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{
    printf("Capstone: 10k-connection HTTP/1.0 server\n");
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for Capstone: 10k-connection HTTP/1.0 server passed.\n");
    return 0;
}
```
