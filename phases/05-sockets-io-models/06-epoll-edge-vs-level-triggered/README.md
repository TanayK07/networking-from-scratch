# 06. epoll: edge vs level triggered

## 1. Problem

In real networking systems, epoll: edge vs level triggered is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that epoll: edge vs level triggered involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

epoll: edge vs level triggered is a core concept in Sockets & I/O Models. Understanding it requires grasping both the design philosophy and the implementation details.

epoll: edge-triggered vs level-triggered:

```
  Level-triggered (default):
    Notifies as long as condition holds (fd is readable)
    Safe but may cause redundant wakeups

  Edge-triggered (EPOLLET):
    Notifies only on state change (new data arrives)
    Must drain entire buffer (read until EAGAIN)
    Higher performance but easy to miss events
...
Scales O(1) with number of monitored fds (vs O(n) for select/poll).
Kernel maintains a ready-list using callbacks on each fd.
...
```c
/*
 * epoll_edge_vs_level_triggered.c -- epoll: edge vs level triggered
 * Compile: gcc -Wall -O2 -o epoll_edge_vs_level_triggered epoll_edge_vs_level_triggered.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from epoll: edge vs level triggered";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for epoll: edge vs level triggered passed.\n");
    return 0;
}
```
...
1. **\u2605** Parse a hex dump of a real epoll: edge vs level triggered packet and identify every field manually.
...
5. **\u2605\u2605** Write a pcap analyzer that reads capture files and decodes epoll: edge vs level triggered packets with full field breakdown.
