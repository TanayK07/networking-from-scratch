...
{
    if (!ctx) return -1;
    if (ctx->next_fd >= NFS_SP_MAX_FDS) return -1;
...
{
    if (!ctx || fd < 0 || fd >= NFS_SP_MAX_FDS) return -1;
...
{
    if (!set || fd < 0 || fd >= NFS_SP_MAX_FDS) return 0;
...
{
    if (!ctx) return -1;
    if (nfds < 0 || nfds > NFS_SP_MAX_FDS) return -1;
    ...
    if (readfds) *readfds = read_result;
    if (writefds) *writefds = write_result;
...
{
    switch (event) {
    case NFS_POLLIN:    return "POLLIN";
    case NFS_POLLOUT:   return "POLLOUT";
    case NFS_POLLERR:   return "POLLERR";
    case NFS_POLLHUP:   return "POLLHUP";
    case NFS_POLLNVAL:  return "POLLNVAL";
