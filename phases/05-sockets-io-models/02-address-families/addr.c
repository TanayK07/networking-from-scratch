                      ...
                      char *out, size_t out_sz)
{
    ...
    case AF_INET:
        if (sa_len >= sizeof(struct sockaddr_in))
            return nfs_addr_format_inet((const struct sockaddr_in *)sa,
                                        out, out_sz);
        break;
    case AF_INET6:
        if (sa_len >= sizeof(struct sockaddr_in6))
            return nfs_addr_format_inet6((const struct sockaddr_in6 *)sa,
                                         out, out_sz);
        break;
    case AF_UNIX:
        if (sa_len >= sizeof(sa_family_t))
            return nfs_addr_format_unix((const struct sockaddr_un *)sa,
                                        out, out_sz);
        break;
    default:
        break;
...
int nfs_addr_cmp_inet6(const struct sockaddr_in6 *a, const struct sockaddr_in6 *b)
{
    ...
    if (memcmp(&a->sin6_addr, &b->sin6_addr, 16) != 0) return 1;
