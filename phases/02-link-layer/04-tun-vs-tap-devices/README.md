# 04. TUN vs TAP devices

## 1. Problem

In real networking systems, tun vs tap devices is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that tun vs tap devices involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

TUN vs TAP devices is a core concept in Link Layer. Understanding it requires grasping both the design philosophy and the implementation details.

TUN vs TAP devices:

```
  TUN (Layer 3):
  App <-> /dev/net/tun <-> [IP packets] <-> kernel routing

  TAP (Layer 2):
  App <-> /dev/net/tun <-> [Ethernet frames] <-> kernel bridge/switch
```

Created via: open("/dev/net/tun") + ioctl(fd, TUNSETIFF, &ifr)
- IFF_TUN: create TUN device (L3, IP packets)
- IFF_TAP: create TAP device (L2, Ethernet frames)
- IFF_NO_PI: no packet info header (recommended)

Used for: VPNs (OpenVPN, WireGuard), userspace TCP/IP stacks, testing.
...
```c
/*
 * tun_vs_tap_devices.c -- TUN vs TAP devices
 * Compile: gcc -Wall -O2 -o tun_vs_tap_devices tun_vs_tap_devices.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from TUN vs TAP devices";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for TUN vs TAP devices passed.\n");
    return 0;
}
```
...
1. **★** Parse a hex dump of a real tun vs tap devices packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes tun vs tap devices packets with full field breakdown.
