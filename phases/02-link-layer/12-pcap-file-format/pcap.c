...
int nfs_pcap_write_packet(FILE *fp, uint32_t ts_sec, uint32_t ts_usec,
...
int nfs_pcap_read_packet(FILE *fp, struct nfs_pcap_pkt_hdr *pkt_hdr,
...
void nfs_pcap_format_header(const struct nfs_pcap_global_hdr *h,
                            char *buf, size_t sz)
{
    if (!h || !buf || sz == 0) return;
    snprintf(buf, sz,
             "PCAP magic=0x%08x version=%u.%u snaplen=%u linktype=%u",
             h->magic, h->version_major, h->version_minor,
             h->snaplen, h->linktype);
}
