...
In real networking systems, pcap file format is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that pcap file format involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

PCAP file format is a core concept in Link Layer. Understanding it requires grasping both the design philosophy and the implementation details.

PCAP file format (used by tcpdump, Wireshark):

```
  Global Header (24 bytes):
  +----------+----------+----------+----------+----------+----------+
  | Magic    | Major    | Minor    | TZ Off   | Sigfigs  | Snap Len |
  | (4B)     | Ver (2B) | Ver (2B) | (4B)     | (4B)     | (4B)     |
  +----------+----------+----------+----------+----------+----------+
  | Link     |
  | Type(4B) |
  +----------+
...
Magic: 0xa1b2c3d4 (LE) or 0xd4c3b2a1 (BE)
Link type: 1 = Ethernet, 113 = Linux cooked capture
...
```c
/*
 * pcap_file_format.c -- PCAP file format
 * Compile: gcc -Wall -O2 -o pcap_file_format pcap_file_format.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from PCAP file format";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for PCAP file format passed.\n");
    return 0;
}
```
...
1. **★** Parse a hex dump of a real pcap file format packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes pcap file format packets with full field breakdown.
