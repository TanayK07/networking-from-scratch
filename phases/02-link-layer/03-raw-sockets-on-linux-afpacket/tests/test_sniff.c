...
static void test_parsed_frame_size(void) {
    ...
    ASSERT_EQ(sizeof(((struct nfs_parsed_frame *)0)->dst), 6, "parsed_frame dst is 6 bytes");
    ASSERT_EQ(sizeof(((struct nfs_parsed_frame *)0)->src), 6, "parsed_frame src is 6 bytes");
}
...
static void test_parse_too_short(void) {
    ...
    ASSERT_EQ(nfs_parse_eth_frame(tiny, sizeof(tiny), &pf), -1, "reject < 14 bytes");
}

static void test_parse_exact_header(void) {
    /* Exactly 14 bytes: valid header, zero-length payload. */
    const uint8_t frame[14] = {
        0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x0a,
        0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x86, 0xdd /* IPv6 */
    };
...
static void test_parse_null_buf(void) {
    struct nfs_parsed_frame pf;
    ASSERT_EQ(nfs_parse_eth_frame(NULL, 100, &pf), -1, "reject NULL buf");
}

static void test_parse_null_out(void) {
    uint8_t frame[14] = {0};
    ASSERT_EQ(nfs_parse_eth_frame(frame, sizeof(frame), NULL), -1, "reject NULL out");
}
