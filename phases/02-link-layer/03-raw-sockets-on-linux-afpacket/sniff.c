...
int nfs_parse_eth_frame(const uint8_t *buf, size_t len, struct nfs_parsed_frame *out) {
    if (!buf || !out)
        return -1;
    if (len < NFS_ETH_HLEN)
        return -1;
...
char *nfs_format_mac(const uint8_t mac[NFS_ETH_ALEN], char *buf, size_t buflen) {
    ...
    if (!mac || !buf || buflen < 18)
        return NULL;

    snprintf(buf, buflen, "%02x:%02x:%02x:%02x:%02x:%02x", mac[0], mac[1], mac[2], mac[3], mac[4],
             mac[5]);
    return buf;
}
...
ssize_t nfs_send_raw_frame(int sockfd, const char *ifname, const uint8_t *frame, size_t frame_len) {
    ...
    ssize_t sent = sendto(sockfd, frame, frame_len, 0, (struct sockaddr *)&dest, sizeof(dest));
    if (sent < 0) {
