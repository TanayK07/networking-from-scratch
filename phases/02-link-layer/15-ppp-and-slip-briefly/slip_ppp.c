...
int nfs_slip_encode(const uint8_t *data, size_t len,
...
int nfs_slip_decode(const uint8_t *frame, size_t frame_len,
                    uint8_t *out, size_t out_sz)
{
    if (!frame || !out || frame_len < 2) return -1;
    ...
    while (i < frame_len - 1) { /* stop before trailing END */
        if (frame[i] == NFS_SLIP_ESC) {
            i++;
            if (i >= frame_len - 1) return -1; /* truncated escape */
            switch (frame[i]) {
...
int nfs_ppp_frame_build(uint16_t protocol,
                        const uint8_t *payload, size_t payload_len,
                        uint8_t *out, size_t out_sz)
{
    if (!out || (!payload && payload_len > 0)) return -1;
    if (payload_len > NFS_PPP_MAX_PAYLOAD) return -1;
...
int nfs_ppp_frame_parse(const uint8_t *data, size_t len,
                        struct nfs_ppp_frame *frame)
{
    ...
    if (computed != frame->fcs) return -1;
...
{
    switch (protocol) {
    case NFS_PPP_PROTO_IPV4: return "IPv4";
    case NFS_PPP_PROTO_IPV6: return "IPv6";
    case NFS_PPP_PROTO_LCP:  return "LCP";
    case NFS_PPP_PROTO_IPCP: return "IPCP";
