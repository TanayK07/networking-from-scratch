...
int main(void) {
    ...
    int slip_len = nfs_slip_encode(ip_pkt, sizeof(ip_pkt), slip_frame, sizeof(slip_frame));
    printf("SLIP encoded (%d bytes):        ", slip_len);
    hex_print(slip_frame, (size_t)slip_len);

    uint8_t decoded[64];
    int dec_len = nfs_slip_decode(slip_frame, (size_t)slip_len, decoded, sizeof(decoded));
    printf("SLIP decoded (%d bytes):        ", dec_len);
    hex_print(decoded, (size_t)dec_len);
    printf("Roundtrip OK: %s\n\n",
           (dec_len == (int)sizeof(ip_pkt) && memcmp(decoded, ip_pkt, sizeof(ip_pkt)) == 0) ? "yes"
                                                                                            : "no");
    ...
    uint8_t ppp_buf[128];
    int ppp_len = nfs_ppp_frame_build(NFS_PPP_PROTO_IPV4, (const uint8_t *)msg, strlen(msg),
    ...
    if (nfs_ppp_frame_parse(ppp_buf, (size_t)ppp_len, &parsed) == 0) {
        ...
        printf("  Protocol: 0x%04X (%s)\n", parsed.protocol,
               nfs_ppp_protocol_name(parsed.protocol));
