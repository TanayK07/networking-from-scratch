...
In real networking systems, vlans (802.1q) is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that vlans (802.1q) involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

VLANs (802.1Q) is a core concept in Link Layer. Understanding it requires grasping both the design philosophy and the implementation details.

802.1Q VLAN tag inserted after source MAC:

```
 +--------+--------+---------+-----------+---------+-----+
 | Dst(6) | Src(6) | 0x8100  | TCI (2B)  |EtherType| ... |
 +--------+--------+---------+-----------+---------+-----+
                     TPID      |PCP|D|  VID  |
                               3b  1b  12 bits
 ...
 VID: VLAN ID (0-4095, 0 and 4095 reserved, 4094 usable)
```

Trunk port: carries tagged frames for multiple VLANs
Access port: strips/adds tags, sends untagged frames to end hosts
Native VLAN: untagged frames on a trunk belong to this VID
...
```c
/*
 * vlans_8021q.c -- VLANs (802.1Q)
 * Compile: gcc -Wall -O2 -o vlans_8021q vlans_8021q.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from VLANs (802.1Q)";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for VLANs (802.1Q) passed.\n");
    return 0;
}
```
...
1. **★** Parse a hex dump of a real vlans packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes vlans packets with full field breakdown.
