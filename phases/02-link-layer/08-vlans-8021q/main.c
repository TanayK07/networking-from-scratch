...
int main(void)
{
    ...
    printf("Original frame: %zu bytes, tagged=%d\n",
           sizeof(frame), nfs_vlan_is_tagged(frame, sizeof(frame)));

    /* Insert VLAN tag: VID=100, PCP=3. */
    uint8_t tagged[64];
    int tlen = nfs_vlan_tag_insert(frame, sizeof(frame), 100, 3,
                                    tagged, sizeof(tagged));
    if (tlen > 0) {
        printf("Tagged frame:   %d bytes, tagged=%d\n",
               tlen, nfs_vlan_is_tagged(tagged, (size_t)tlen));
    ...
    uint8_t stripped[64];
    uint16_t vid;
    int slen = nfs_vlan_tag_strip(tagged, (size_t)tlen,
                                   stripped, sizeof(stripped), &vid);
    if (slen > 0) {
        printf("Stripped frame: %d bytes, tagged=%d, VID=%u\n",
               slen, nfs_vlan_is_tagged(stripped, (size_t)slen), vid);
    }
