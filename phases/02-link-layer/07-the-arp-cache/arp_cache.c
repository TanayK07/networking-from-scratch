...
const char *nfs_arp_state_name(int state) {
    switch (state) {
    case NFS_ARP_INCOMPLETE:
        return "INCOMPLETE";
    case NFS_ARP_REACHABLE:
        return "REACHABLE";
    case NFS_ARP_STALE:
        return "STALE";
    case NFS_ARP_FAILED:
...
void nfs_arp_cache_format(const struct nfs_arp_cache *c, char *buf, size_t sz) {
    ...
    for (size_t i = 0; i < c->count && off < sz - 1; i++) {
        ...
        int n = snprintf(buf + off, sz - off, "%u.%u.%u.%u  %02x:%02x:%02x:%02x:%02x:%02x  %s\n", a,
                         b, cc, d, e->mac[0], e->mac[1], e->mac[2], e->mac[3], e->mac[4], e->mac[5],
