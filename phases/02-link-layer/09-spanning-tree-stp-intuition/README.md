# 09. Spanning Tree (STP) intuition

## 1. Problem

Before diving into implementation details, you need a clear mental model of how spanning tree (stp) intuition works. Without understanding the design rationale and tradeoffs, you will struggle to debug real systems or make informed architectural decisions.

Many engineers work with link layer concepts daily without understanding the mechanics underneath. This lesson builds the foundational understanding that makes everything else click.

## 2. Theory

Spanning Tree (STP) intuition is a core concept in Link Layer. Understanding it requires grasping both the design philosophy and the implementation details.
...
```
  Bridge A ------- Bridge B
     \              /
      \            /
       \          /
        Bridge C        <-- loop! broadcast storm without STP

  With STP:
  Bridge A ------- Bridge B   (root bridge elected)
     \              /
      \            / <-- one port blocked
       \          /
        Bridge C
...
2. Each non-root bridge finds shortest path to root (Root Port)
3. Each segment has one Designated Port (forwarding)
4. All other ports are Blocked (no forwarding)
5. BPDU (Bridge Protocol Data Unit) messages maintain the tree

## 3. Math / Spec

The theoretical foundation for spanning tree (stp) intuition involves understanding the tradeoffs between different design choices. Key metrics include:
...
```python
"""
spanning_tree_stp_intuition.py -- Spanning Tree (STP) intuition
"""
import struct
import socket

class PacketBuilder:
    """Build and parse protocol packets for Spanning Tree (STP) intuition."""
...
def demo():
    payload = b"Hello from Spanning Tree (STP) intuition"
...
1. **★** Parse a hex dump of a real spanning tree packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes spanning tree packets with full field breakdown.
