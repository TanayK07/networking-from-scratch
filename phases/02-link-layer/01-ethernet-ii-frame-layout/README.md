# 01. Ethernet II frame layout

## 1. Problem

In real networking systems, ethernet ii frame layout is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that ethernet ii frame layout involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

Ethernet II frame layout is a core concept in Link Layer. Understanding it requires grasping both the design philosophy and the implementation details.

Ethernet II frame layout:

```
 +---------------+---------------+-----------+-----------------+-----+
 |  Dst MAC (6B) |  Src MAC (6B) | EtherType |   Payload       | FCS |
 |               |               |   (2B)    |  (46-1500B)     |(4B) |
 +---------------+---------------+-----------+-----------------+-----+
```

EtherType values: 0x0800=IPv4, 0x0806=ARP, 0x86DD=IPv6, 0x8100=802.1Q
Minimum frame: 64 bytes (excl. preamble). Payload padded to 46 bytes if shorter.
Wire overhead: preamble(8) + header(14) + FCS(4) + IFG(12) = 38 bytes.
...
```c
/*
 * ethernet_ii_frame_layout.c -- Ethernet II frame layout
 * Compile: gcc -Wall -O2 -o ethernet_ii_frame_layout ethernet_ii_frame_layout.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from Ethernet II frame layout";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for Ethernet II frame layout passed.\n");
    return 0;
}
```
...
1. **★** Parse a hex dump of a real ethernet ii frame layout packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes ethernet ii frame layout packets with full field breakdown.
