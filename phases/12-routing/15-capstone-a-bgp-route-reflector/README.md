# 15. Capstone: a BGP route reflector

## 1. Problem

This capstone brings together everything from the Routing: BGP, OSPF, Control Plane phase into a single working project. You will build a complete, functional implementation of a BGP route reflector that demonstrates mastery of all concepts covered so far.
...
The a BGP route reflector project integrates multiple components built in previous lessons. The architecture follows a layered design where each module handles one responsibility:
...
```python
"""
capstone_a_bgp_route_reflector.py -- Capstone: Capstone: a BGP route reflector
...
if __name__ == '__main__':
    ap = argparse.ArgumentParser(description='Capstone: a BGP route reflector')
