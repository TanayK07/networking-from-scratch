# 16. Capstone: an OSPF area-0 simulator

## 1. Problem

This capstone brings together everything from the Routing: BGP, OSPF, Control Plane phase into a single working project. You will build a complete, functional implementation of an OSPF area-0 simulator that demonstrates mastery of all concepts covered so far.
...
The an OSPF area-0 simulator project integrates multiple components built in previous lessons. The architecture follows a layered design where each module handles one responsibility:
...
```python
"""
capstone_an_ospf_area0_simulator.py -- Capstone: Capstone: an OSPF area-0 simulator
...
if __name__ == '__main__':
    ap = argparse.ArgumentParser(description='Capstone: an OSPF area-0 simulator')
