# 09. Send path end to end

## 1. Problem

Before diving into implementation details, you need a clear mental model of how send path end to end works. Without understanding the design rationale and tradeoffs, you will struggle to debug real systems or make informed architectural decisions.
...
Send path end to end is a fundamental concept in linux kernel networking internals. Understanding it requires grasping both the high-level design philosophy and the low-level details.
...
When studying send path end to end, pay attention to:
...
The theoretical foundation for send path end to end involves understanding the tradeoffs between different design choices. Key metrics include:
...
```c
/*
 * send_path_end_to_end.c -- Send path end to end
 * Compile: gcc -Wall -O2 -o send_path_end_to_end send_path_end_to_end.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from Send path end to end";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for Send path end to end passed.\n");
    return 0;
}
```
...
1. **\u2605** Parse a hex dump of a real send path end to end packet and identify every field manually.
...
5. **\u2605\u2605** Write a pcap analyzer that reads capture files and decodes send path end to end packets with full field breakdown.
