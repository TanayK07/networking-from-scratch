# 05. NAPI: schedule, poll, complete

## 1. Problem

In real networking systems, napi: schedule, poll, complete is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that napi: schedule, poll, complete involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

NAPI: schedule, poll, complete is a core concept in Linux Kernel Networking Internals. Understanding it requires grasping both the design philosophy and the implementation details.

NAPI (New API) flow:

```
  1. First packet arrives -> NIC raises IRQ
  2. IRQ handler:
     - Disable further IRQs from this NIC
     - napi_schedule(&napi)  -> add to poll list
  3. Softirq (NET_RX_SOFTIRQ) runs:
     - napi_poll() callback:
       - Read up to 'budget' packets from NIC ring buffer
       - Build sk_buffs, pass up stack via netif_receive_skb()
       - If ring drained: napi_complete_done() -> re-enable IRQs
       - If budget exhausted: return budget (will be re-scheduled)
  4. Budget: net.core.dev_weight (default 64 packets per poll)
```

This hybrid interrupt-then-poll model avoids interrupt storms under
high packet rates. At line rate, the NIC is polled continuously
with no interrupts at all.
...
```c
/*
 * napi_schedule_poll_complete.c -- NAPI: schedule, poll, complete
 * Compile: gcc -Wall -O2 -o napi_schedule_poll_complete napi_schedule_poll_complete.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from NAPI: schedule, poll, complete";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for NAPI: schedule, poll, complete passed.\n");
    return 0;
}
```
...
1. **★** Parse a hex dump of a real napi: schedule, poll, complete packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes napi: schedule, poll, complete packets with full field breakdown.
