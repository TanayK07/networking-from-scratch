# 24. Capstone: 1% packet-loss injection LKM

## 1. Problem

This capstone brings together everything from the Linux Kernel Networking Internals phase into a single working project. You will build a complete, functional implementation of 1% packet-loss injection LKM that demonstrates mastery of all concepts covered so far.
...
The 1% packet-loss injection LKM project integrates multiple components built in previous lessons. The architecture follows a layered design where each module handles one responsibility:
...
```c
/*
 * capstone_1_packetloss_injection_lkm.c -- Capstone: Capstone: 1% packet-loss injection LKM
 *
 * Architecture:
 ...
 *
 * Compile: gcc -Wall -O2 -o capstone_1_packetloss_injection_lkm capstone_1_packetloss_injection_lkm.c -lpthread
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{
    printf("Capstone: 1% packet-loss injection LKM\n");
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for Capstone: 1% packet-loss injection LKM passed.\n");
    return 0;
}
```
