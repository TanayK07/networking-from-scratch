...
truesize: total memory charged (sk_buff + data buffer + shared_info).
Allocated from kmem_cache slab. Reference counted via skb_get/kfree_skb.
...
1. **★** Parse a hex dump of a real the sk_buff: head, data, tail, end packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes the sk_buff: head, data, tail, end packets with full field breakdown.
