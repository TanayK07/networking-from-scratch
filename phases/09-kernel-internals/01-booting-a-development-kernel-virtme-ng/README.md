# 01. Booting a development kernel (virtme-ng)

## 1. Problem

Before diving into implementation details, you need a clear mental model of how booting a development kernel (virtme-ng) works. Without understanding the design rationale and tradeoffs, you will struggle to debug real systems or make informed architectural decisions.

Many engineers work with linux kernel networking internals concepts daily without understanding the mechanics underneath. This lesson builds the foundational understanding that makes everything else click.

## 2. Theory

Booting a development kernel (virtme-ng) is a fundamental concept in linux kernel networking internals. Understanding it requires grasping both the high-level design philosophy and the low-level details.
...
When studying booting a development kernel (virtme-ng), pay attention to:
...
The theoretical foundation for booting a development kernel (virtme-ng) involves understanding the tradeoffs between different design choices. Key metrics include:
...
```c
/*
 * booting_a_development_kernel_virtmeng.c -- Booting a development kernel (virtme-ng)
 * Compile: gcc -Wall -O2 -o booting_a_development_kernel_virtmeng booting_a_development_kernel_virtmeng.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from Booting a development kernel (virtme-ng)";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for Booting a development kernel (virtme-ng) passed.\n");
    return 0;
}
```
...
1. **★** Parse a hex dump of a real booting a development kernel packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes booting a development kernel packets with full field breakdown.
