# 19. GSO/TSO/LRO offloads

## 1. Problem

Before diving into implementation details, you need a clear mental model of how gso/tso/lro offloads works. Without understanding the design rationale and tradeoffs, you will struggle to debug real systems or make informed architectural decisions.
...
GSO/TSO/LRO offloads is a fundamental concept in linux kernel networking internals. Understanding it requires grasping both the high-level design philosophy and the low-level details.
...
When studying gso/tso/lro offloads, pay attention to:
...
The theoretical foundation for gso/tso/lro offloads involves understanding the tradeoffs between different design choices. Key metrics include:
...
```python
"""
gsotsolro_offloads.py -- GSO/TSO/LRO offloads
"""
import struct
import socket

class PacketBuilder:
    """Build and parse protocol packets for GSO/TSO/LRO offloads."""
...
def demo():
    payload = b"Hello from GSO/TSO/LRO offloads"
...
1. **★** Parse a hex dump of a real gso/tso/lro offloads packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes gso/tso/lro offloads packets with full field breakdown.
