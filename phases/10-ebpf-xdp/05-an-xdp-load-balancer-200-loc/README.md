# 05. An XDP load balancer (~200 LOC)

## 1. Problem

In real networking systems, an xdp load balancer (~200 loc) is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that an xdp load balancer (~200 loc) involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

An XDP load balancer (~200 LOC) is a core concept in eBPF, XDP, and Kernel Bypass. Understanding it requires grasping both the design philosophy and the implementation details.

XDP load balancer architecture (~200 lines):

```
  Client          XDP LB            Backend servers
  1.2.3.4  --->  10.0.0.1  --->   192.168.1.{10,11,12}
                    |
                    | XDP program:
                    | 1. Parse Ethernet + IP + TCP/UDP headers
                    | 2. Hash 5-tuple -> backend index
                    | 3. Rewrite dst IP + MAC
                    | 4. Recompute checksums
                    | 5. XDP_TX (send modified packet back out)
                    |
  BPF maps:
  - backends (array): index -> {IP, MAC}
  - connections (hash): 5-tuple -> backend (for sticky sessions)
```

Consistent hashing ensures flow affinity. Maglev or ring hashing
for minimal disruption when backends are added/removed.
...
```c
/*
 * an_xdp_load_balancer_200_loc.c -- An XDP load balancer (~200 LOC)
 * Compile: gcc -Wall -O2 -o an_xdp_load_balancer_200_loc an_xdp_load_balancer_200_loc.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from An XDP load balancer (~200 LOC)";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for An XDP load balancer (~200 LOC) passed.\n");
    return 0;
}
```
...
1. **\u2605** Parse a hex dump of a real an xdp load balancer packet and identify every field manually.
...
5. **\u2605\u2605** Write a pcap analyzer that reads capture files and decodes an xdp load balancer packets with full field breakdown.
