# 04. XDP basics: PASS, DROP, TX, REDIRECT

## 1. Problem

In real networking systems, xdp basics: pass, drop, tx, redirect is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that xdp basics: pass, drop, tx, redirect involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

XDP basics: PASS, DROP, TX, REDIRECT is a core concept in eBPF, XDP, and Kernel Bypass. Understanding it requires grasping both the design philosophy and the implementation details.

XDP (eXpress Data Path) hook point and actions:
...
```
  NIC receives packet
       ...
       v
  +----------+
  | XDP prog |   <- earliest programmable hook (before sk_buff allocation)
  +----+-----+
       |
       +-> XDP_DROP      (discard immediately - DDoS defense)
       +-> XDP_PASS      (continue to normal stack)
       +-> XDP_TX        (bounce back out same NIC)
       +-> XDP_REDIRECT  (send to another NIC or AF_XDP socket)
       +-> XDP_ABORTED   (error, drop + trace)
```

Modes:
  - Native (driver): XDP runs in driver's NAPI poll, before sk_buff
  - Generic (skb): XDP runs on sk_buff, slower but works on all NICs
  - Offloaded: XDP runs on NIC hardware (SmartNIC/FPGA)

Performance: 24 Mpps (million packets per second) per core with XDP_DROP.
...
```c
/*
 * xdp_basics_pass_drop_tx_redirect.c -- XDP basics: PASS, DROP, TX, REDIRECT
 * Compile: gcc -Wall -O2 -o xdp_basics_pass_drop_tx_redirect xdp_basics_pass_drop_tx_redirect.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from XDP basics: PASS, DROP, TX, REDIRECT";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for XDP basics: PASS, DROP, TX, REDIRECT passed.\n");
    return 0;
}
```
...
1. **★** Parse a hex dump of a real xdp basics: pass, drop, tx, redirect packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes xdp basics: pass, drop, tx, redirect packets with full field breakdown.
