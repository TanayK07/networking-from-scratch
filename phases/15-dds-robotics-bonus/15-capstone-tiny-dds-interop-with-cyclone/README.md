# 15. Capstone: tiny DDS interop with Cyclone

## 1. Problem

This capstone brings together everything from the BONUS: DDS, RTPS, and Robotics Middleware phase into a single working project. You will build a complete, functional implementation of tiny DDS interop with Cyclone that demonstrates mastery of all concepts covered so far.
...
The tiny DDS interop with Cyclone project integrates multiple components built in previous lessons. The architecture follows a layered design where each module handles one responsibility:
...
```c
/*
 * capstone_tiny_dds_interop_with_cyclone.c -- Capstone: Capstone: tiny DDS interop with Cyclone
 *
 * Architecture:
 ...
 *
 * Compile: gcc -Wall -O2 -o capstone_tiny_dds_interop_with_cyclone capstone_tiny_dds_interop_with_cyclone.c -lpthread
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{
    printf("Capstone: tiny DDS interop with Cyclone\n");
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for Capstone: tiny DDS interop with Cyclone passed.\n");
    return 0;
}
```
