# 25. Capstone: a robotics demo on PREEMPT_RT

## 1. Problem

This capstone brings together everything from the BONUS: DDS, RTPS, and Robotics Middleware phase into a single working project. You will build a complete, functional implementation of a robotics demo on PREEMPT_RT that demonstrates mastery of all concepts covered so far.
...
The a robotics demo on PREEMPT_RT project integrates multiple components built in previous lessons. The architecture follows a layered design where each module handles one responsibility:
...
```c
/*
 * capstone_a_robotics_demo_on_preemptrt.c -- Capstone: Capstone: a robotics demo on PREEMPT_RT
 *
 * Architecture:
 ...
 *
 * Compile: gcc -Wall -O2 -o capstone_a_robotics_demo_on_preemptrt capstone_a_robotics_demo_on_preemptrt.c -lpthread
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{
    printf("Capstone: a robotics demo on PREEMPT_RT\n");
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for Capstone: a robotics demo on PREEMPT_RT passed.\n");
    return 0;
}
```
