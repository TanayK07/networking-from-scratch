# 09. SPDP — Participant Discovery

## 1. Problem

In real networking systems, spdp — participant discovery is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that spdp — participant discovery involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

SPDP — Participant Discovery requires careful attention to binary protocol formats and state management. The implementation must handle network byte order (big-endian) correctly and validate all input before processing.
...
```c
/*
 * spdp_participant_discovery.c -- SPDP — Participant Discovery
 * Compile: gcc -Wall -O2 -o spdp_participant_discovery spdp_participant_discovery.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from SPDP — Participant Discovery";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for SPDP — Participant Discovery passed.\n");
    return 0;
}
```
...
1. **★** Parse a hex dump of a real spdp — participant discovery packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes spdp — participant discovery packets with full field breakdown.
