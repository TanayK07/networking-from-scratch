                  ...
                  struct nfs_ntp_packet *out)
{
    ...
    uint32_t tmp;
    memcpy(&tmp, &buf[16], 4); out->ref_ts.seconds    = ntohl(tmp);
    memcpy(&tmp, &buf[20], 4); out->ref_ts.fraction   = ntohl(tmp);
    memcpy(&tmp, &buf[24], 4); out->origin_ts.seconds  = ntohl(tmp);
    memcpy(&tmp, &buf[28], 4); out->origin_ts.fraction = ntohl(tmp);
    memcpy(&tmp, &buf[32], 4); out->recv_ts.seconds    = ntohl(tmp);
    memcpy(&tmp, &buf[36], 4); out->recv_ts.fraction   = ntohl(tmp);
    memcpy(&tmp, &buf[40], 4); out->xmit_ts.seconds    = ntohl(tmp);
    memcpy(&tmp, &buf[44], 4); out->xmit_ts.fraction   = ntohl(tmp);
                  ...
                  const struct nfs_ntp_packet *pkt)
{
    ...
    uint32_t tmp;
    tmp = htonl(pkt->root_delay);      memcpy(&buf[4],  &tmp, 4);
    tmp = htonl(pkt->root_dispersion); memcpy(&buf[8],  &tmp, 4);
    tmp = htonl(pkt->ref_id);          memcpy(&buf[12], &tmp, 4);

    tmp = htonl(pkt->ref_ts.seconds);     memcpy(&buf[16], &tmp, 4);
    tmp = htonl(pkt->ref_ts.fraction);    memcpy(&buf[20], &tmp, 4);
    tmp = htonl(pkt->origin_ts.seconds);  memcpy(&buf[24], &tmp, 4);
    tmp = htonl(pkt->origin_ts.fraction); memcpy(&buf[28], &tmp, 4);
    tmp = htonl(pkt->recv_ts.seconds);    memcpy(&buf[32], &tmp, 4);
    tmp = htonl(pkt->recv_ts.fraction);   memcpy(&buf[36], &tmp, 4);
    tmp = htonl(pkt->xmit_ts.seconds);    memcpy(&buf[40], &tmp, 4);
    tmp = htonl(pkt->xmit_ts.fraction);   memcpy(&buf[44], &tmp, 4);
