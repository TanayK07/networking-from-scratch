...
nfs_http_conn_state_t nfs_http_conn_on_response(struct nfs_http_conn *conn) {
    ...
    if (conn->max_requests > 0 && conn->request_count >= conn->max_requests) {
...
int nfs_http_conn_should_close(const struct nfs_http_conn *conn) {
    ...
    if (conn->state == NFS_HTTP_CONN_CLOSE)
        return 1;
...
const char *nfs_http_conn_response_header(const struct nfs_http_conn *conn) {
    ...
    if (conn->version == NFS_HTTP_VER_10 && conn->keep_alive_explicit && !conn->close_explicit) {
        return "keep-alive";
    }
...
int nfs_http_pipeline_enqueue(struct nfs_http_conn *conn, const char *method, const char *uri) {
    if (!conn || !method || !uri)
        return -1;
    if (conn->pipeline_count >= NFS_HTTP_MAX_PIPELINE)
        return -1;

    struct nfs_http_pipeline_entry *e = &conn->pipeline[conn->pipeline_tail];

    size_t mlen = strlen(method);
    size_t ulen = strlen(uri);
    if (mlen >= NFS_HTTP_MAX_METHOD || ulen >= NFS_HTTP_MAX_URI)
        return -1;
