...
int nfs_h2_frame_parse(const uint8_t *buf, size_t len, struct nfs_h2_frame *out) {
    if (!buf || !out || len < NFS_H2_FRAME_HEADER_SIZE)
        return -1;

    /* Length: 3 bytes big-endian */
    out->length = ((uint32_t)buf[0] << 16) | ((uint32_t)buf[1] << 8) | ((uint32_t)buf[2]);
...
int nfs_h2_frame_build(uint8_t *buf, size_t out_sz, uint8_t type, uint8_t flags, uint32_t stream_id,
...
int nfs_h2_settings_parse(const uint8_t *payload, size_t len, struct nfs_h2_setting *out,
                          size_t max) {
    if (!payload && len > 0)
        return -1;
    if (!out && max > 0)
...
int nfs_h2_settings_build(uint8_t *buf, size_t out_sz, const struct nfs_h2_setting *settings,
                          size_t count) {
