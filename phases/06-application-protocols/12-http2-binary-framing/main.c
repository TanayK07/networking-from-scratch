...
int main(void)
{
    ...
    int slen = nfs_h2_settings_build(settings_payload, sizeof(settings_payload),
                                     settings, 2);
    if (slen < 0) {
    ...
    int total = nfs_h2_frame_build(buf, sizeof(buf),
                                   NFS_H2_SETTINGS, 0, 0,
                                   settings_payload, (uint32_t)slen);
    if (total < 0) {
    ...
    total = nfs_h2_frame_build(buf, sizeof(buf),
                               NFS_H2_DATA, NFS_H2_FLAG_END_STREAM, 1,
                               (const uint8_t *)data, (uint32_t)strlen(data));
    printf("\nBuilt %d-byte DATA frame on stream 1\n", total);

    if (nfs_h2_frame_parse(buf, (size_t)total, &frame) == 0) {
        printf("  Type: %s, Flags: 0x%02x, Stream: %u, Payload: %.*s\n",
               nfs_h2_frame_type_name(frame.type), frame.flags,
               frame.stream_id, frame.length, frame.payload);
    }
