...
int nfs_vhost_add(struct nfs_vhost_table *table, const char *hostname, const char *docroot,
                  int is_default) {
    if (!table || !hostname || !docroot)
        return -1;
    if (table->count >= NFS_VHOST_MAX_HOSTS)
        return -1;
...
int nfs_vhost_add_alias(struct nfs_vhost_table *table, int host_idx, const char *alias) {
    if (!table || !alias)
        return -1;
    if (host_idx < 0 || host_idx >= table->count)
        return -1;

    struct nfs_vhost *vh = &table->hosts[host_idx];
    if (vh->alias_count >= NFS_VHOST_MAX_ALIASES)
        return -1;

    size_t alen = strlen(alias);
    if (alen >= NFS_VHOST_MAX_HOSTNAME)
        return -1;
...
const struct nfs_vhost *nfs_vhost_lookup(const struct nfs_vhost_table *table,
                                         const char *hostname) {
    if (!table || !hostname)
        return NULL;
...
const struct nfs_vhost *nfs_vhost_get_default(const struct nfs_vhost_table *table) {
    if (!table || table->default_idx < 0)
        return NULL;
    if (table->default_idx >= table->count)
        return NULL;
    return &table->hosts[table->default_idx];
}
...
int nfs_vhost_parse_host_value(const char *value, char *hostname, size_t hostname_sz,
                               uint16_t *port) {
    if (!value || !hostname || hostname_sz == 0)
        return -1;
...
int nfs_vhost_extract_host(const char *request, size_t request_len, char *host_out,
                           size_t host_out_sz) {
    if (!request || !host_out || host_out_sz == 0)
        return -1;
    ...
    while (pos < end) {
        ...
        if (line_len > 5 && (pos[0] == 'H' || pos[0] == 'h') && (pos[1] == 'o' || pos[1] == 'O') &&
            (pos[2] == 's' || pos[2] == 'S') && (pos[3] == 't' || pos[3] == 'T') && pos[4] == ':') {
            ...
            return nfs_vhost_parse_host_value(raw_val, host_out, host_out_sz, NULL);
        }
...
const struct nfs_vhost *nfs_vhost_route(const struct nfs_vhost_table *table, const char *request,
    ...
    if (nfs_vhost_extract_host(request, request_len, hostname, sizeof(hostname)) != 0) {
