...
int main(void) {
    ...
    printf("\n=== Lookups ===\n");
    const char *hostnames[] = {"example.com", "www.example.com", "blog.example.com",
                               "api.example.com", "unknown.com"};
    for (int i = 0; i < 5; i++) {
    ...
    const char *req_no_host = "GET / HTTP/1.1\r\n"
                              "\r\n";
    printf("Without Host: %s\n",
           nfs_vhost_validate_request(req_no_host, strlen(req_no_host)) == 0 ? "valid" : "invalid");

    /* HTTP/1.0 without Host is OK */
    const char *req_10 = "GET / HTTP/1.0\r\n"
                         "\r\n";
    printf("HTTP/1.0 without Host: %s\n",
           nfs_vhost_validate_request(req_10, strlen(req_10)) == 0 ? "valid" : "invalid");
