...
size_t nfs_chunked_parse_hex(const char *hex, size_t len)
{
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        /* Overflow check */
        if (val > (size_t)-1 / 16) return (size_t)-1;
...
int nfs_chunked_encode_chunk(const uint8_t *data, size_t data_len,
...
{
    if (!out || out_sz < 5) return -1;
    /* "0\r\n\r\n" */
    memcpy(out, "0\r\n\r\n", 5);
    return 5;
}

int nfs_chunked_encode(const uint8_t *data, size_t data_len,
                       size_t chunk_size,
                       uint8_t *out, size_t out_sz)
{
    if (!out) return -1;
    ...
    while (offset < data_len) {
        ...
        int written = nfs_chunked_encode_chunk(data + offset, csz,
                                                out + pos, out_sz - pos);
...
int nfs_chunk_parse_next(const uint8_t *data, size_t data_len,
                         struct nfs_chunk *chunk)
{
    if (!data || !chunk || data_len == 0) return -1;
    ...
    if (chunk_size == (size_t)-1) return -1;
    ...
    if (consumed + chunk_size + 2 > data_len) return -1;

    chunk->size = chunk_size;
    chunk->data = data + consumed;

    /* Verify trailing CRLF after data */
    if (data[consumed + chunk_size] != '\r' ||
        data[consumed + chunk_size + 1] != '\n') {
        return -1;
    }
...
int nfs_chunked_decode(const uint8_t *data, size_t data_len,
                       struct nfs_chunked_decoded *decoded)
{
    if (!data || !decoded || !decoded->data) return -1;
        ...
        if (chunk.size == 0) {
            ...
            while (pos < data_len) {
                ...
                if (decoded->trailer_count < NFS_CHUNKED_MAX_TRAILERS) {
                    const uint8_t *colon = memchr(data + pos, ':',
                                                   line_len);
                    if (colon) {
                        struct nfs_chunked_trailer *t =
                            &decoded->trailers[decoded->trailer_count];
                        ...
                        if (nlen < NFS_CHUNKED_MAX_HDR_NAME) {
                            ...
                            while (vstart < crlf && *vstart == ' ') vstart++;
