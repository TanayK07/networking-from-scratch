...
int main(void)
{
    ...
    uint8_t buf[256];
    int total = nfs_ws_build_frame(NFS_WS_TEXT, 1, 0, NULL,
    ...
    if (total > 0) {
        ...
        if (nfs_ws_parse_frame(buf, (size_t)total, &frame, &hdr_len) == 0) {
            ...
            printf("  Opcode:  0x%x (%s)\n", frame.opcode,
                   nfs_ws_opcode_str(frame.opcode));
            printf("  Masked:  %d\n", frame.masked);
    ...
    total = nfs_ws_build_frame(NFS_WS_TEXT, 1, 1, mask,
                               (const uint8_t *)msg, strlen(msg),
                               buf, sizeof(buf));
    if (total > 0) {
        ...
        if (nfs_ws_parse_frame(buf, (size_t)total, &frame, &hdr_len) == 0) {
            ...
            nfs_ws_mask_payload(payload, (size_t)frame.payload_len,
                                frame.mask_key);
