# 26. Capstone: HTTP CONNECT proxy

## 1. Problem

This capstone brings together everything from the Application Protocols phase into a single working project. You will build a complete, functional implementation of HTTP CONNECT proxy that demonstrates mastery of all concepts covered so far.
...
The HTTP CONNECT proxy project integrates multiple components built in previous lessons. The architecture follows a layered design where each module handles one responsibility:
...
```c
/*
 * capstone_http_connect_proxy.c -- Capstone: Capstone: HTTP CONNECT proxy
 *
 * Architecture:
 ...
 *
 * Compile: gcc -Wall -O2 -o capstone_http_connect_proxy capstone_http_connect_proxy.c -lpthread
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{
    printf("Capstone: HTTP CONNECT proxy\n");
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for Capstone: HTTP CONNECT proxy passed.\n");
    return 0;
}
```
