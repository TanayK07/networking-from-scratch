# 08. HTTP/1.0 and HTTP/1.1 parser

## 1. Problem

In real networking systems, http/1.0 and http/1.1 parser is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that http/1.0 and http/1.1 parser involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

HTTP/1.0 and HTTP/1.1 parser is a core concept in Application Protocols. Understanding it requires grasping both the design philosophy and the implementation details.
...
- Empty line (CRLF CRLF) separates headers from body
- Content-Length or Transfer-Encoding: chunked determines body length
...
```c
/*
 * http10_and_http11_parser.c -- HTTP/1.0 and HTTP/1.1 parser
 * Compile: gcc -Wall -O2 -o http10_and_http11_parser http10_and_http11_parser.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from HTTP/1.0 and HTTP/1.1 parser";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for HTTP/1.0 and HTTP/1.1 parser passed.\n");
    return 0;
}
```
...
1. **★** Parse a hex dump of a real http/1.0 and http/1.1 parser packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes http/1.0 and http/1.1 parser packets with full field breakdown.
