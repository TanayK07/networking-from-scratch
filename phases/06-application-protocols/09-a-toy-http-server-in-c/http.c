...
int nfs_http_request_parse(const char *data, size_t data_len, struct nfs_http_request *req) {
    ...
    if (method_len == 0 || method_len >= NFS_HTTP_MAX_METHOD)
        return -1;
    ...
    if (uri_len == 0 || uri_len >= NFS_HTTP_MAX_URI)
        return -1;
    ...
    if (ver_len == 0 || ver_len >= NFS_HTTP_MAX_VERSION)
        return -1;
    ...
    while (pos < end) {
        ...
        if (name_len == 0 || name_len >= NFS_HTTP_MAX_HDR_NAME)
            return -1;
        ...
        if (val_len >= NFS_HTTP_MAX_HDR_VALUE)
            return -1;

        if (req->header_count >= NFS_HTTP_MAX_HEADERS)
            return -1;
...
const char *nfs_http_request_get_header(const struct nfs_http_request *req, const char *name) {
    if (!req || !name)
        return NULL;
...
void nfs_http_response_init(struct nfs_http_response *resp, uint16_t status_code) {
    if (!resp)
        return;
    ...
    if (tlen >= sizeof(resp->status_text))
        tlen = sizeof(resp->status_text) - 1;
    memcpy(resp->status_text, text, tlen);
    resp->status_text[tlen] = '\0';
}

int nfs_http_response_add_header(struct nfs_http_response *resp, const char *name,
                                 const char *value) {
    if (!resp || !name || !value)
        return -1;
    if (resp->header_count >= NFS_HTTP_MAX_HEADERS)
        return -1;

    struct nfs_http_header *h = &resp->headers[resp->header_count];
    size_t nlen = strlen(name);
    size_t vlen = strlen(value);
    if (nlen >= NFS_HTTP_MAX_HDR_NAME || vlen >= NFS_HTTP_MAX_HDR_VALUE)
        return -1;
...
void nfs_http_response_set_body(struct nfs_http_response *resp, const uint8_t *body,
                                size_t body_len) {
    if (!resp)
        return;
    resp->body = body;
...
int nfs_http_response_build(const struct nfs_http_response *resp, char *out, size_t out_sz) {
    if (!resp || !out || out_sz == 0)
        return -1;
