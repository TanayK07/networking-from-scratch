...
int main(void) {
    ...
    int qlen = nfs_dns_query_build("www.example.com", NFS_DNS_TYPE_A, 0x1234, buf, sizeof(buf));
    if (qlen < 0) {
    ...
    int consumed = nfs_dns_name_decode(encoded, (size_t)elen, 0, decoded, sizeof(decoded));
    if (consumed > 0) {
    ...
    if (nfs_dns_response_parse(resp_buf, off, &resp) == 0) {
        ...
        printf("Question: %s type=%s\n", resp.qname, nfs_dns_type_str(resp.qtype));
        printf("Answers: %u\n", resp.ancount);
        for (uint16_t i = 0; i < resp.ancount; i++) {
            struct nfs_dns_rr *rr = &resp.answers[i];
            printf("  %s type=%s TTL=%u rdlen=%u", rr->name, nfs_dns_type_str(rr->type), rr->ttl,
                   rr->rdlength);
            if (rr->type == NFS_DNS_TYPE_A && rr->rdlength == 4) {
                printf(" -> %u.%u.%u.%u", rr->rdata[0], rr->rdata[1], rr->rdata[2], rr->rdata[3]);
            }
            printf("\n");
        }
    }
