...
{
    if (!data || !out || out_sz == 0 || offset >= data_len) return -1;
    ...
    while (cur < data_len) {
        ...
        if (len == 0) {
            if (!jumped) bytes_consumed = (int)(cur - offset + 1);
            break;
        }

        if ((len & 0xC0) == 0xC0) {
            if (cur + 1 >= data_len) return -1;
            if (!jumped) bytes_consumed = (int)(cur - offset + 2);
...
{
    if (!zone || !origin) return -1;
    memset(zone, 0, sizeof(*zone));
    size_t len = strlen(origin);
    if (len >= NFS_DNS_MAX_NAME) return -1;
    memcpy(zone->origin, origin, len + 1);
    return 0;
}

int nfs_dns_zone_add_rr(struct nfs_dns_zone *zone,
                        const char *name, uint16_t type, uint16_t rclass,
                        uint32_t ttl, const uint8_t *rdata, uint16_t rdlength)
{
    if (!zone || !name) return -1;
    if (zone->rr_count >= NFS_DNS_MAX_RR) return -1;
    if (rdlength > 0 && !rdata) return -1;
    if (rdlength > sizeof(((struct nfs_dns_rr *)0)->rdata)) return -1;

    struct nfs_dns_rr *rr = &zone->rrs[zone->rr_count];
    size_t nlen = strlen(name);
    if (nlen >= NFS_DNS_MAX_NAME) return -1;
...
int nfs_dns_zone_add_a(struct nfs_dns_zone *zone,
                       const char *name, uint32_t ttl,
                       uint8_t a, uint8_t b, uint8_t c, uint8_t d)
{
    uint8_t rdata[4] = {a, b, c, d};
    return nfs_dns_zone_add_rr(zone, name, NFS_DNS_TYPE_A,
                                NFS_DNS_CLASS_IN, ttl, rdata, 4);
}

int nfs_dns_zone_add_ns(struct nfs_dns_zone *zone,
                        const char *name, uint32_t ttl,
                        const char *nsname)
{
    if (!nsname) return -1;
    ...
    if (elen < 0) return -1;
    return nfs_dns_zone_add_rr(zone, name, NFS_DNS_TYPE_NS,
                                NFS_DNS_CLASS_IN, ttl,
                                encoded, (uint16_t)elen);
}

int nfs_dns_zone_add_soa(struct nfs_dns_zone *zone,
                         const char *name, uint32_t ttl,
                         const struct nfs_dns_soa *soa)
{
    ...
    if (pos + 20 > sizeof(rdata)) return -1;
    uint32_t vals[5] = {
        htonl(soa->serial), htonl(soa->refresh),
        htonl(soa->retry), htonl(soa->expire),
        htonl(soa->minimum)
    };
    memcpy(rdata + pos, vals, 20);
    pos += 20;

    return nfs_dns_zone_add_rr(zone, name, NFS_DNS_TYPE_SOA,
                                NFS_DNS_CLASS_IN, ttl,
                                rdata, (uint16_t)pos);
}
...
int nfs_dns_zone_lookup(const struct nfs_dns_zone *zone,
                        const char *qname, uint16_t qtype,
                        struct nfs_dns_lookup_result *result)
{
    if (!zone || !qname || !result) return -1;
...
static int write_rr_wire(const struct nfs_dns_rr *rr,
                         uint8_t *out, size_t out_sz)
{
    ...
    if (pos + 10 + rr->rdlength > out_sz) return -1;

    uint16_t tmp;
    tmp = htons(rr->type);    memcpy(out + pos, &tmp, 2); pos += 2;
    tmp = htons(rr->rclass);  memcpy(out + pos, &tmp, 2); pos += 2;
    uint32_t t = htonl(rr->ttl);
    memcpy(out + pos, &t, 4); pos += 4;
    tmp = htons(rr->rdlength); memcpy(out + pos, &tmp, 2); pos += 2;
    if (rr->rdlength > 0) {
...
int nfs_dns_zone_build_response(const struct nfs_dns_zone *zone,
                                const uint8_t *query, size_t query_len,
                                uint8_t *out, size_t out_sz)
{
    if (!zone || !query || !out) return -1;
    if (query_len < NFS_DNS_HEADER_SIZE) return -1;
    ...
    int consumed = nfs_dns_name_decode(query, query_len, NFS_DNS_HEADER_SIZE,
                                        qname, sizeof(qname));
    if (consumed < 0) return -1;

    size_t qoff = NFS_DNS_HEADER_SIZE + (size_t)consumed;
    if (qoff + 4 > query_len) return -1;
    ...
    if (qflags & 0x0100) rflags |= 0x0100; /* RD */
    rflags |= rcode;
    ...
    uint16_t tmp;
    tmp = htons(qtype);  memcpy(out + pos, &tmp, 2); pos += 2;
    tmp = htons(qclass); memcpy(out + pos, &tmp, 2); pos += 2;
