# 12. Capstone: a virtual wire

## 1. Problem

This capstone brings together everything from the Bits & Wires phase into a single working project. You will build a complete, functional implementation of a virtual wire that demonstrates mastery of all concepts covered so far.
...
Capstone: a virtual wire is a core concept in Bits & Wires. Understanding it requires grasping both the design philosophy and the implementation details.

Virtual wire architecture:

```
  +----------+     +---------------+     +----------+
  | Sender   |---->| Virtual Wire  |---->| Receiver |
  | (NIC A)  |     |  - BER        |     | (NIC B)  |
  |          |<----|  - drop rate  |<----|          |
  +----------+     |  - delay      |     +----------+
                   |  - reorder    |
...
Impairment model:
- BER (Bit Error Rate): flip each bit with probability p
- Drop: discard frame with probability p_drop
- Delay: base_delay + N(0, sigma^2) jitter
- Reorder: swap adjacent frames with probability p_reorder
...
```c
/*
 * capstone_a_virtual_wire.c -- Capstone: Capstone: a virtual wire
 *
 * Architecture:
 ...
 *
 * Compile: gcc -Wall -O2 -o capstone_a_virtual_wire capstone_a_virtual_wire.c -lpthread
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{
    printf("Capstone: a virtual wire\n");
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for Capstone: a virtual wire passed.\n");
    return 0;
}
```
