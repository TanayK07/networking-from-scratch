# 10. Framing: byte stuffing, bit stuffing

## 1. Problem

In real networking systems, framing: byte stuffing, bit stuffing is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that framing: byte stuffing, bit stuffing involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

Framing: byte stuffing, bit stuffing is a core concept in Bits & Wires. Understanding it requires grasping both the design philosophy and the implementation details.
...
Original:  [01] [7E] [03] [7D] [05]
Stuffed:   [7E] [01] [7D 5E] [03] [7D 5D] [05] [7E]
            flag                                  flag
```

Bit stuffing (HDLC): flag = 01111110, insert 0 after five consecutive 1s.

COBS (Consistent Overhead Byte Stuffing): replaces each zero byte with
distance-to-next-zero. Overhead: max 1 byte per 254 data bytes (~0.4%).
...
```c
/*
 * framing_byte_stuffing_bit_stuffing.c -- Framing: byte stuffing, bit stuffing
 * Compile: gcc -Wall -O2 -o framing_byte_stuffing_bit_stuffing framing_byte_stuffing_bit_stuffing.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from Framing: byte stuffing, bit stuffing";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for Framing: byte stuffing, bit stuffing passed.\n");
    return 0;
}
```
...
1. **★** Parse a hex dump of a real framing: byte stuffing, bit stuffing packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes framing: byte stuffing, bit stuffing packets with full field breakdown.
