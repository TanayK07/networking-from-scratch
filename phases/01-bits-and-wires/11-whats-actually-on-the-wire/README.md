# 11. What's actually on the wire

## 1. Problem

Before diving into implementation details, you need a clear mental model of how what's actually on the wire works. Without understanding the design rationale and tradeoffs, you will struggle to debug real systems or make informed architectural decisions.
...
What's actually on the wire is a fundamental concept in bits & wires. Understanding it requires grasping both the high-level design philosophy and the low-level details.
...
When studying what's actually on the wire, pay attention to:
...
The theoretical foundation for what's actually on the wire involves understanding the tradeoffs between different design choices. Key metrics include:
...
```python
"""
whats_actually_on_the_wire.py -- What's actually on the wire
"""
import struct
import socket

class PacketBuilder:
    """Build and parse protocol packets for What's actually on the wire."""
...
def demo():
    payload = b"Hello from What's actually on the wire"
...
1. **★** Parse a hex dump of a real what's actually on the wire packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes what's actually on the wire packets with full field breakdown.
