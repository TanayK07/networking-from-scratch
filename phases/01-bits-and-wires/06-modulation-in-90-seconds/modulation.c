...
{
    if (level < -2.0) return 0;       /* -3 -> 00 */
    if (level < 0.0)  return 1;       /* -1 -> 01 */
    if (level < 2.0)  return 3;       /* +1 -> 11 */
    return 2;                          /* +3 -> 10 */
}
...
nfs_symbol_t nfs_qpsk_modulate(int dibit)
{
    ...
    switch (dibit) {
    case 0: s.i = +INV_SQRT2; s.q = +INV_SQRT2; break;
    case 1: s.i = -INV_SQRT2; s.q = +INV_SQRT2; break;
    case 3: s.i = -INV_SQRT2; s.q = -INV_SQRT2; break;
    case 2: s.i = +INV_SQRT2; s.q = -INV_SQRT2; break;
    }
    return s;
}
...
{
    nfs_symbol_t s = {0.0, 0.0};
    if (nibble < 0 || nibble > 15) return s;
...
int nfs_qam16_demodulate(nfs_symbol_t symbol)
{
    ...
    if (qi <= -2.0) qi = -3.0;
    else if (qi <= 0.0) qi = -1.0;
    else if (qi <= 2.0) qi = 1.0;
    else qi = 3.0;

    if (qq <= -2.0) qq = -3.0;
    else if (qq <= 0.0) qq = -1.0;
    else if (qq <= 2.0) qq = 1.0;
    else qq = 3.0;
...
{
    switch (scheme) {
    case NFS_MOD_OOK:   return 1;
    case NFS_MOD_BPSK:  return 1;
    case NFS_MOD_QPSK:  return 2;
    case NFS_MOD_QAM16: return 4;
    default:             return -1;
    }
}
