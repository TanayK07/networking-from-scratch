# 04. Bit fields and packed structs

## 1. Problem

In real networking systems, bit fields and packed structs is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that bit fields and packed structs involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

Bit fields and packed structs is a core concept in Bits & Wires. Understanding it requires grasping both the design philosophy and the implementation details.

IPv4 first byte layout (network order):
...
In memory on LE machine, byte = 0x45:
  version = (byte >> 4) & 0xF  = 4
  ihl     = byte & 0xF         = 5
...
```c
/*
 * bit_fields_and_packed_structs.c -- Bit fields and packed structs
 * Compile: gcc -Wall -O2 -o bit_fields_and_packed_structs bit_fields_and_packed_structs.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from Bit fields and packed structs";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for Bit fields and packed structs passed.\n");
    return 0;
}
```
...
1. **\u2605** Parse a hex dump of a real bit fields and packed structs packet and identify every field manually.
...
5. **\u2605\u2605** Write a pcap analyzer that reads capture files and decodes bit fields and packed structs packets with full field breakdown.
