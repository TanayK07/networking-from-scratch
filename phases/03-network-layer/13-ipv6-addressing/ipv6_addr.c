...
const char *nfs_ipv6_addr_type_str(const uint8_t addr[16]) {
    if (nfs_ipv6_addr_is_loopback(addr))
        return "loopback";
    if (nfs_ipv6_addr_is_unspecified(addr))
        return "unspecified";
    if (nfs_ipv6_addr_is_multicast(addr))
        return "multicast";
    if (nfs_ipv6_addr_is_link_local(addr))
        return "link-local";
    if (nfs_ipv6_addr_is_ula(addr))
        return "unique local";
    if (nfs_ipv6_addr_is_global_unicast(addr))
