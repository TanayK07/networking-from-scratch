...
int main(void)
{
    ...
    nfs_nat_outbound(&nat, host_a, 4000, server, 80, NFS_PROTO_TCP,
                     &new_ip, &new_port);
    ...
    nfs_nat_outbound(&nat, host_b, 5000, server, 80, NFS_PROTO_TCP,
                     &new_ip, &new_port);
    printf("  NAT rewrites to: ");
    print_ip(new_ip);
    printf(":%u\n", new_port);
    ...
    uint32_t orig_ip;
    uint16_t orig_port;
    int rc = nfs_nat_inbound(&nat, server, 80, 10000, NFS_PROTO_TCP,
                             &orig_ip, &orig_port);
    if (rc == 0) {
    ...
    rc = nfs_nat_inbound(&nat, server, 80, 9999, NFS_PROTO_TCP,
                         &orig_ip, &orig_port);
