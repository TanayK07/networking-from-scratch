...
void nfs_nat_init(struct nfs_nat_table *t, size_t capacity, uint32_t external_ip,
                  uint16_t port_start) {
...
static struct nfs_nat_entry *find_entry(struct nfs_nat_table *t, uint32_t src_ip, uint16_t src_port,
                                        uint32_t dst_ip, uint16_t dst_port, uint8_t proto) {
    for (size_t i = 0; i < t->count; i++) {
        struct nfs_nat_entry *e = &t->entries[i];
        if (e->internal_ip == src_ip && e->internal_port == src_port && e->remote_ip == dst_ip &&
            e->remote_port == dst_port && e->protocol == proto)
            return e;
    }
    return NULL;
}

int nfs_nat_outbound(struct nfs_nat_table *t, uint32_t src_ip, uint16_t src_port, uint32_t dst_ip,
                     uint16_t dst_port, uint8_t proto, uint32_t *new_src_ip,
                     uint16_t *new_src_port) {
    /* Check for existing mapping (same flow reuses port). */
    struct nfs_nat_entry *e = find_entry(t, src_ip, src_port, dst_ip, dst_port, proto);
    if (e) {
...
int nfs_nat_inbound(struct nfs_nat_table *t, uint32_t src_ip, uint16_t src_port, uint16_t dst_port,
                    uint8_t proto, uint32_t *orig_dst_ip, uint16_t *orig_dst_port) {
    for (size_t i = 0; i < t->count; i++) {
        struct nfs_nat_entry *e = &t->entries[i];
        if (e->external_port == dst_port && e->remote_ip == src_ip && e->remote_port == src_port &&
            e->protocol == proto) {
...
void nfs_nat_format(const struct nfs_nat_table *t, char *buf, size_t sz) {
    size_t off = 0;
    int n = snprintf(buf + off, sz - off, "NAT Table (%zu/%zu entries):\n", t->count, t->capacity);
    if (n < 0)
        return;
    off += (size_t)n;
    ...
    for (size_t i = 0; i < t->count && off < sz; i++) {
        ...
        n = snprintf(buf + off, sz - off, "  %s:%u -> %s:%u  (remote %s:%u, proto %u)\n", int_ip,
                     e->internal_port, ext_ip, e->external_port, rem_ip, e->remote_port,
                     e->protocol);
        if (n < 0)
            return;
        off += (size_t)n;
