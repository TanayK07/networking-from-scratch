...
int nfs_ipv4_parse(const uint8_t *buf, size_t len, struct nfs_ipv4_hdr *out) {
    ...
    out->src_addr = ((uint32_t)buf[12] << 24) |
                    ((uint32_t)buf[13] << 16) |
                    ((uint32_t)buf[14] <<  8) |
                    ((uint32_t)buf[15]);

    /* Bytes 16-19: destination address. */
    out->dst_addr = ((uint32_t)buf[16] << 24) |
                    ((uint32_t)buf[17] << 16) |
                    ((uint32_t)buf[18] <<  8) |
                    ((uint32_t)buf[19]);
...
size_t nfs_ipv4_build(const struct nfs_ipv4_hdr *hdr, uint8_t *buf, size_t len) {
    ...
    uint16_t flags_frag = (uint16_t)(((uint16_t)hdr->flags << 13) |
...
const char *nfs_ipv4_protocol_name(uint8_t proto) {
    switch (proto) {
        case NFS_IPPROTO_ICMP: return "ICMP";
        case NFS_IPPROTO_TCP:  return "TCP";
        case NFS_IPPROTO_UDP:  return "UDP";
        case NFS_IPPROTO_IPV6: return "IPv6-in-IPv4";
        case  2: return "IGMP";
        case 47: return "GRE";
        case 50: return "ESP";
        case 51: return "AH";
