...
static int build_lla_option(uint8_t type, const uint8_t *mac,
                            uint8_t *out, size_t out_sz)
{
    /* Option: type(1) + length(1) + MAC(6) = 8 bytes, length field = 1 (units of 8) */
    if (out_sz < 8) return -1;
    out[0] = type;
    out[1] = 1;  /* 1 * 8 = 8 bytes */
    memcpy(out + 2, mac, 6);
    return 8;
...
int nfs_ndp_build_ns(const uint8_t *target,
                     ...
                     uint8_t *out, size_t out_sz)
{
    ...
    if (src_mac) {
        int opt_len = build_lla_option(NFS_NDP_OPT_SRC_LLA, src_mac,
                                       out + pos, out_sz - pos);
        if (opt_len < 0) return -1;
        pos += (size_t)opt_len;
    }
...
int nfs_ndp_build_na(const uint8_t *target, uint32_t flags,
                     const uint8_t *tgt_mac,
                     uint8_t *out, size_t out_sz)
{
    if (!target || !out) return -1;
    ...
    if (tgt_mac) {
        int opt_len = build_lla_option(NFS_NDP_OPT_TGT_LLA, tgt_mac,
                                       out + pos, out_sz - pos);
        if (opt_len < 0) return -1;
        pos += (size_t)opt_len;
    }
...
int nfs_ndp_build_rs(const uint8_t *src_mac,
                     uint8_t *out, size_t out_sz)
{
    if (!out) return -1;
...
int nfs_ndp_build_ra(uint8_t cur_hop_limit, uint8_t flags,
...
int nfs_ndp_parse_type(const uint8_t *data, size_t len)
{
    ...
    uint8_t type = data[0];
    if (type < NFS_NDP_RS || type > NFS_NDP_REDIRECT) return -1;

    /* Validate minimum lengths */
    size_t min_len = 0;
    switch (type) {
    case NFS_NDP_RS:       min_len = sizeof(struct nfs_ndp_rs); break;
    case NFS_NDP_RA:       min_len = sizeof(struct nfs_ndp_ra); break;
    case NFS_NDP_NS:       min_len = sizeof(struct nfs_ndp_ns); break;
    case NFS_NDP_NA:       min_len = sizeof(struct nfs_ndp_na); break;
    case NFS_NDP_REDIRECT: min_len = 40; break; /* minimum redirect size */
    default: return -1;
    }
...
{
    if (!data || !ns) return -1;
    if (len < sizeof(struct nfs_ndp_ns)) return -1;
    if (data[0] != NFS_NDP_NS || data[1] != 0) return -1;
...
{
    if (!data || !na) return -1;
    if (len < sizeof(struct nfs_ndp_na)) return -1;
    if (data[0] != NFS_NDP_NA || data[1] != 0) return -1;
...
{
    if (!data || !rs) return -1;
    if (len < sizeof(struct nfs_ndp_rs)) return -1;
    if (data[0] != NFS_NDP_RS || data[1] != 0) return -1;
...
{
    if (!data || !ra) return -1;
    if (len < sizeof(struct nfs_ndp_ra)) return -1;
    if (data[0] != NFS_NDP_RA || data[1] != 0) return -1;
...
int nfs_ndp_option_next(const uint8_t *opt_data, size_t opt_len,
                        size_t *offset, struct nfs_ndp_parsed_option *opt)
{
    if (!opt_data || !offset || !opt) return -1;
    if (*offset + 2 > opt_len) return -1;
    ...
    if (*offset + total > opt_len) return -1;
