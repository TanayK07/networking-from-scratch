...
int main(void)
{
    ...
    /* Build NA */
    n = nfs_ndp_build_na(target, NFS_NDP_NA_FLAG_S | NFS_NDP_NA_FLAG_O,
                         mac, buf, sizeof(buf));
    ...
    if (nfs_ndp_parse_na(buf, (size_t)n, &na) == 0) {
        printf("  Flags: R=%d S=%d O=%d\n",
               ...
               (na.flags & NFS_NDP_NA_FLAG_S) ? 1 : 0,
               (na.flags & NFS_NDP_NA_FLAG_O) ? 1 : 0);
    }
    ...
    /* Build RA */
    n = nfs_ndp_build_ra(64, NFS_NDP_RA_FLAG_M, 1800, 30000, 1000,
                         mac, buf, sizeof(buf));
