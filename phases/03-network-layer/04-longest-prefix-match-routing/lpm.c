...
void nfs_route_table_format(const struct nfs_route_table *t, char *buf, size_t sz) {
    ...
    n = snprintf(buf + offset, sz - offset, "%-18s %-18s %s\n", "Prefix", "Next Hop", "Iface");
    if (n < 0 || (size_t)n >= sz - offset)
        return;
    offset += (size_t)n;
    ...
    for (size_t i = 0; i < t->count; i++) {
        ...
        n = snprintf(buf + offset, sz - offset, "%u.%u.%u.%u/%-5u %u.%u.%u.%u      %d\n",
                     (p >> 24) & 0xFF, (p >> 16) & 0xFF, (p >> 8) & 0xFF, p & 0xFF, r->prefix_len,
                     (nh >> 24) & 0xFF, (nh >> 16) & 0xFF, (nh >> 8) & 0xFF, nh & 0xFF,
                     r->interface_id);
        if (n < 0 || (size_t)n >= sz - offset)
            return;
        offset += (size_t)n;
