# 22. Capstone: a userspace IP router

## 1. Problem

This capstone brings together everything from the Network Layer: IPv4, IPv6, ICMP phase into a single working project. You will build a complete, functional implementation of a userspace IP router that demonstrates mastery of all concepts covered so far.
...
The a userspace IP router project integrates multiple components built in previous lessons. The architecture follows a layered design where each module handles one responsibility:
...
```c
/*
 * capstone_a_userspace_ip_router.c -- Capstone: Capstone: a userspace IP router
 *
 * Architecture:
 ...
 *
 * Compile: gcc -Wall -O2 -o capstone_a_userspace_ip_router capstone_a_userspace_ip_router.c -lpthread
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{
    printf("Capstone: a userspace IP router\n");
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for Capstone: a userspace IP router passed.\n");
    return 0;
}
```
