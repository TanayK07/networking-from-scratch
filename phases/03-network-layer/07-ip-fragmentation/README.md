...
In real networking systems, ip fragmentation is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that ip fragmentation involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

IP fragmentation is a core concept in Network Layer: IPv4, IPv6, ICMP. Understanding it requires grasping both the design philosophy and the implementation details.

IP fragmentation:

```
  Original packet: [IP hdr | 4000 bytes payload]
  MTU = 1500 bytes -> max payload per fragment = 1480 (must be multiple of 8)

  Fragment 1: [IP hdr (MF=1, offset=0)    | 1480 bytes]
  Fragment 2: [IP hdr (MF=1, offset=1480) | 1480 bytes]
  Fragment 3: [IP hdr (MF=0, offset=2960) | 1040 bytes]
```

Fields used: Identification (same for all fragments), MF (More Fragments flag),
Fragment Offset (in units of 8 bytes).

IPv6: routers NEVER fragment. Only the source can fragment.
Path MTU Discovery (RFC 1191): set DF bit, get ICMP "Fragmentation Needed".
...
```c
/*
 * ip_fragmentation.c -- IP fragmentation
 * Compile: gcc -Wall -O2 -o ip_fragmentation ip_fragmentation.c
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
{
    uint8_t pkt[1500];
    const char *msg = "Hello from IP fragmentation";
...
int main(void)
{
    ...
    test_empty_payload();
    printf("All tests for IP fragmentation passed.\n");
    return 0;
}
```
...
1. **\u2605** Parse a hex dump of a real ip fragmentation packet and identify every field manually.
...
5. **\u2605\u2605** Write a pcap analyzer that reads capture files and decodes ip fragmentation packets with full field breakdown.
