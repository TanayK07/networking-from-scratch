...
int nfs_rtable_remove(struct nfs_rtable *t, uint32_t dest, uint8_t prefix)
{
    ...
    for (size_t i = 0; i < t->count; i++) {
        if (t->entries[i].dest == masked_dest &&
            t->entries[i].prefix_len == prefix) {
...
{
    return snprintf(buf, sz, "%u.%u.%u.%u",
    ...
    for (size_t i = 0; i < t->count; i++) {
        ...
        if (e->prefix_len == 0) {
            n = snprintf(buf + total, sz - (size_t)total,
                         "default via %s dev if%d metric %u",
                         gw, e->iface, e->metric);
        } else if (e->flags & NFS_RTF_GATEWAY) {
            n = snprintf(buf + total, sz - (size_t)total,
                         "%s/%u via %s dev if%d metric %u",
                         dest, e->prefix_len, gw, e->iface, e->metric);
        } else {
            n = snprintf(buf + total, sz - (size_t)total,
                         "%s/%u dev if%d metric %u",
                         dest, e->prefix_len, e->iface, e->metric);
        }
        if (n < 0) return -1;
        total += n;
