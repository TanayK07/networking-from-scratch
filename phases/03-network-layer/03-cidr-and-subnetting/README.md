# 03. CIDR and subnetting

## 1. Problem

In real networking systems, cidr and subnetting is a critical component that you encounter constantly. If you cannot implement it correctly from first principles, you will be at the mercy of library bugs, misconfigurations, and subtle protocol violations that are nearly impossible to debug without deep understanding.

The challenge is that cidr and subnetting involves precise binary layouts, strict protocol rules, and edge cases that only manifest under specific network conditions. Getting even one byte wrong means packets are silently dropped or connections mysteriously fail.

## 2. Theory

CIDR and subnetting is a core concept in Network Layer: IPv4, IPv6, ICMP. Understanding it requires grasping both the design philosophy and the implementation details.

CIDR (Classless Inter-Domain Routing):

```
  10.0.0.0/24  =  network: 10.0.0.0, mask: 255.255.255.0
                   hosts: 10.0.0.1 - 10.0.0.254 (254 usable)

  Subnetting example:
  10.0.0.0/24  split into four /26 subnets:
    10.0.0.0/26    hosts: .1-.62     (62 hosts)
    10.0.0.64/26   hosts: .65-.126   (62 hosts)
    10.0.0.128/26  hosts: .129-.190  (62 hosts)
    10.0.0.192/26  hosts: .193-.254  (62 hosts)
```
...
```python
"""
cidr_and_subnetting.py -- CIDR and subnetting
"""
import struct
import socket

class PacketBuilder:
    """Build and parse protocol packets for CIDR and subnetting."""
...
def demo():
    payload = b"Hello from CIDR and subnetting"
...
1. **\u2605** Parse a hex dump of a real cidr and subnetting packet and identify every field manually.
...
5. **\u2605\u2605** Write a pcap analyzer that reads capture files and decodes cidr and subnetting packets with full field breakdown.
