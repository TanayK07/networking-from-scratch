...
int main(void)
{
    ...
    size_t nfound = 0;
    int rc = nfs_dhcp_parse_options(msg->options, sizeof(msg->options),
                                   opts, 32, &nfound);
    ...
    nfound = 0;
    rc = nfs_dhcp_parse_options(offer.options, sizeof(offer.options),
                                opts, 32, &nfound);
    printf("Parsed OFFER: %s (%zu options)\n",
           rc == 0 ? "OK" : "FAIL", nfound);
    printf("  Your IP: "); print_ip(offer.yiaddr); printf("\n");
    printf("  Server:  "); print_ip(offer.siaddr); printf("\n");
    ...
    for (size_t i = 0; i < nfound; i++) {
        ...
        if (opts[i].code == NFS_DHCP_OPT_MSG_TYPE)
            ...
            printf("  -> %u.%u.%u.%u", opts[i].data[0], opts[i].data[1],
                   opts[i].data[2], opts[i].data[3]);
