# 13. kube-proxy modes: iptables, IPVS, eBPF

## 1. Problem

Before diving into implementation details, you need a clear mental model of how kube-proxy modes: iptables, ipvs, ebpf works. Without understanding the design rationale and tradeoffs, you will struggle to debug real systems or make informed architectural decisions.

Many engineers work with overlays & container networking concepts daily without understanding the mechanics underneath. This lesson builds the foundational understanding that makes everything else click.

## 2. Theory

kube-proxy modes: iptables, IPVS, eBPF is a fundamental concept in overlays & container networking. Understanding it requires grasping both the high-level design philosophy and the low-level details.
...
When studying kube-proxy modes: iptables, ipvs, ebpf, pay attention to:
...
The theoretical foundation for kube-proxy modes: iptables, ipvs, ebpf involves understanding the tradeoffs between different design choices. Key metrics include:
...
```python
"""
kubeproxy_modes_iptables_ipvs_ebpf.py -- kube-proxy modes: iptables, IPVS, eBPF
"""
import struct
import socket

class PacketBuilder:
    """Build and parse protocol packets for kube-proxy modes: iptables, IPVS, eBPF."""
...
def demo():
    payload = b"Hello from kube-proxy modes: iptables, IPVS, eBPF"
...
1. **★** Parse a hex dump of a real kube-proxy modes: iptables, ipvs, ebpf packet and identify every field manually.
...
5. **★★** Write a pcap analyzer that reads capture files and decodes kube-proxy modes: iptables, ipvs, ebpf packets with full field breakdown.
